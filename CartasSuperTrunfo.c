...
int main() {
    ...
    float populacao1,populacao2;                            // População da Cidade (Tipo Double Para Números Grandes)
    ...
    scanf("%1s", estado1); //%1 Para Aceitar apenas 1 caractere                    
    ...
    scanf(" %f", &populacao1); // Lê a população da cidade (tipo float)
    ...
    getchar();  // Limpa o buffer para evitar erro ao ler a string
    printf("Digite o Nome da Cidade: \n");
    scanf(" %[^\n]", nome2); // Lê a linha inteira (Permite espaços no nome)

    printf("Digite a população da cidade:\n");
    scanf(" %f", &populacao2); // Lê a população da cidade (tipo float)
